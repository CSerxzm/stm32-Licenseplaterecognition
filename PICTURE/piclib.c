...
void piclib_init(void)
{
	...
	pic_phy.draw_point=LCD_DrawFRONT_COLOR;	//画点函数实现
	...
	picinfo.lcdwidth=tftlcd_data.width;	//得到LCD的宽度像素
	picinfo.lcdheight=tftlcd_data.height;//得到LCD的高度像素
...
{	
	...
	{
		picinfo.S_Height=tftlcd_data.height;
		picinfo.S_Width=tftlcd_data.width;
		return FALSE;   
	}
	...
	picinfo.S_XOFF=x;
	//文件名传递		 
	temp=FATFS_ScanFilesType((u8*)filename);	//得到文件的类型
	switch(temp)
	{											  
		case T_BMP:
